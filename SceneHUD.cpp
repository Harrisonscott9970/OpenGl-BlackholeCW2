...
// ─── drawEarth ───────────────────────────────────────────────────────────────
// Draws a photorealistic Earth-like planet using the scissor/drawRect system.
// Rendered entirely in 2D: Lambertian ocean, continent shapes derived from
// real spherical coordinates, swirling cloud wisps, and a thick atmosphere limb.
//
// cx, cy  — screen centre of the planet
// ER      — radius in pixels
// alpha   — overall opacity (0 = invisible, 1 = full)
// The step parameter controls quality (3=high, 4=fast, 6=perf)
    ...
    std::function<void(int, int, int, int, float, float, float)> drawRect)
{
    ...
    // step=3 looks great when large, auto-coarsen when tiny
    int step = glm::max(3, 280 / glm::max(1, ER));

    // Light from upper-left: sun direction in screen-space.
    // Normalized: LX^2+LY^2+LZ^2 = 1
    const float LX = -0.36f, LY = -0.54f, LZ = 0.76f;

    // --- Per-pixel loop over the planet disc ---
    for (int row = -ER; row <= ER; row += step)
    {
        ...
        for (int col = -hw; col <= hw; col += step)
        {
            ...
            // Proper spherical UV so patterns don't tile or mirror
            float lon = atan2f(dx, dz);                          // -PI..PI
            float lat = asinf(glm::clamp(dy, -1.0f, 1.0f));     // -PI/2..PI/2
            // Remap to 0..1 for cleaner pattern math
            float u = (lon + 3.14159f) / 6.28318f;   // 0..1
            float v = (lat + 1.5708f) / 3.14159f;   // 0..1 (0=south, 1=north)

            // --- Lighting ---
            float ndotl = glm::clamp(LX * dx + LY * dy + LZ * dz, 0.0f, 1.0f);
            float light = glm::smoothstep(0.02f, 0.35f, ndotl);  // soft terminator
            float limbD = 0.50f + 0.50f * dz;                    // limb darkening
            float shade = light * limbD;

            // --- Continent mask (FBM-like, breaks the sine symmetry) ---
            // Key trick: use u*2PI and v*PI as angles, then combine with
            // different incommensurate frequencies so no visible tiling.
            ...
            // land is roughly -2..2; threshold at ~0.55 gives ~28% land cover
            bool isLand = (land > 0.55f);

            // Polar ice: above 68 deg latitude (v > 0.88 or v < 0.12)
            bool polarIce = (v > 0.88f || v < 0.12f);
            if (polarIce) isLand = true;

            // Sub-biome based on land height and latitude band
            ...
            // --- Cloud mask (different frequencies, no shared axes with land) ---
            ...
            // Clouds denser near equator and mid-latitudes, fewer near poles
            ...
            if (polarIce)
            {
                // Antarctic/Arctic: blue-white ice sheet
            ...
            {
                // Snow-capped rocky peaks: grey with white caps
            ...
            {
                // Sahara/outback sandy orange-tan
            ...
            {
                // High-latitude tundra/taiga: dark olive-green
            ...
            {
                // Tropical/temperate: bright green to olive
            ...
            {
                // Ocean: deep navy with realistic shallow-water tint near coasts
                // and a sharp specular glint on the lit side
                float coastProx = glm::clamp(1.0f - (land - 0.55f) / 0.6f, 0.0f, 1.0f);
                float spec = powf(ndotl, 22.0f) * light;  // sharp sun glint
            ...
            // Cloud overlay: bright white wisps, only on lit side
            if (isCloudy)
            {
    ...
    // --- Atmosphere limb: thin glowing blue ring outside the disc ---
    // Multiple layers from ER to 1.14*ER for a thick scattering glow
    ...
    for (int row = -atmosMax; row <= atmosMax; row += step)
    {
        ...
        if (ow < 1) continue;
        // Fade from inner edge to outer edge
        float edgeFrac = (float)(hw2) / glm::max(1, hw);
        float ha = (edgeFrac) * (1.0f - fabsf(rf) * 0.55f) * 0.70f * alpha;
        // Lit side atmosphere slightly brighter (Rayleigh scattering)
...
void SceneBasic_Uniform::drawOverlayUI()
{
    ...
    if (showHUD) {
    drawBitmapText(margin, topY, "ARRAYS " + std::to_string(collected) + "/" + std::to_string(total), 4, 0.68f, 0.95f, 1.00f);
    ...
    drawBar(width - 220, height - 74, 190, 12, boostEnergy, glm::vec3(0.16f, 0.86f, 1.00f), "BOOST");
    {
        // Red lockout threshold line at 25% on BOOST bar
        int barX = width - 220;
        int barY = height - 74;
        int barW = 190;
        int barH = 12;
        int threshX = barX + 2 + (int)((barW - 4) * 0.25f);
        // Draw a 2-pixel-wide red vertical tick spanning the full bar height
        drawRect(threshX, barY, 2, barH, 1.00f, 0.10f, 0.10f);
    }
    ...
    if (showHUD && !inCockpit)
        ...
        drawBitmapText(margin, 74, "X EVA  SHIFT BOOST", 3, 0.60f, 0.92f, 1.00f);

    // ── World-space markers: cells (normal) or dust clouds (NV) ─────────────────
    ...
    // Helper: project a world point → screen pixel.
    // Always returns true and fills sx/sy. onScreen=true only when the point is
    // actually visible inside the viewport.  For behind-camera or off-screen points
    // sx/sy is clamped to the 24-px inner border so an edge arrow can be placed.
    auto projectWorld = [&](glm::vec3 wp, int& sx, int& sy, bool& onScreen) -> bool
    {
        ...
        if (clip.w <= 0.0f)
        {
            // Behind the camera: negate x/y so the edge arrow appears on the
            // side of the screen you need to turn toward to face the point.
            float iw = (fabsf(clip.w) > 0.0001f) ? (1.0f / -clip.w) : 0.0f;
            nx = -clip.x * iw * 0.5f + 0.5f;
            ny = -clip.y * iw * 0.5f + 0.5f;
            // Push firmly to the nearest edge so the arrow is always on the border
    ...
    // Arrow helper: draws a small right-pointing chevron, rotated by (dx,dy) direction.
    // We approximate rotation via sign-based mirroring — enough for N/S/E/W edges.
    auto drawArrow = [&](int ax, int ay, int dx, int dy, float r, float g, float b)
    {
        // Draw a simple 3-pixel-wide arrow head pointing in (dx,dy) direction
    ...
    if (!nightVisionMode)
    {
        // ── Normal mode: blue square markers above each uncollected energy cell ──
        for (const auto& cell : energyCells)
        {
            ...
            if (!projectWorld(cell.position + glm::vec3(0.0f, 1.2f, 0.0f), sx, sy, onScreen))
                continue;
            if (onScreen) {
                drawFrame(sx - 10, sy - 10, 20, 20, 0.22f, 0.88f, 1.00f);
                drawRect (sx -  2, sy -  2,  4,  4, 1.00f, 1.00f, 1.00f);
            } else {
                // Off-screen: draw edge arrow pointing toward the cell
    ...
    {
        // ── NV mode: WHITE dust-cloud markers (white contrasts NV green screen) ──
        // dustClusterPositions is pre-computed every frame in renderParticles(),
        // so we just read it directly — no filtering of the particles array here.
        const std::vector<glm::vec3>& shown = dustClusterPositions;

        // Always-visible NV status bar — top-right corner.
        // White text on dark panel confirms NV is active and shows cloud count.
        {
        ...
        // Pulse: 0.75..1.00 — never too dim to see on the NV green screen
        ...
        for (const auto& wp : shown)
        {
            ...
            if (onScreen)
            {
                // ── Large WHITE target reticle — impossible to miss on NV green ──
                // Outer box (48×48)
                drawFrame(mx - 24, my - 24, 48, 48, pulse, pulse, pulse);
                // Inner box (32×32) — green tint
                drawFrame(mx - 16, my - 16, 32, 32, 0.20f*pulse, 1.00f*pulse, 0.35f*pulse);
                // Extended cross-hair ticks (white)
                drawRect(mx - 38, my - 2, 14, 4, pulse, pulse, pulse);  // left tick
                drawRect(mx + 24, my - 2, 14, 4, pulse, pulse, pulse);  // right tick
                drawRect(mx -  2, my - 38, 4, 14, pulse, pulse, pulse); // top tick
                drawRect(mx -  2, my + 24, 4, 14, pulse, pulse, pulse); // bottom tick
                // Bright white centre dot (10×10)
                drawRect(mx - 5, my - 5, 10, 10, 1.00f, 1.00f, 1.00f);
                // "DUST CLOUD" label below marker in large text
                drawBitmapText(mx - 24, my + 28, "DUST", 3, 1.00f, 1.00f, 0.70f);
            }
            else
            {
                // ── Off-screen: bold edge arrow pointing toward the cloud ──
                // Determine which edge the clamped position landed on
                bool atLeft   = (mx <= 24 + 2);
                ...
                if (atLeft) {
                    // Arrow pointing left (toward off-screen left)
                    drawRect(mx,      my - 4, 20, 8, pulse, pulse, pulse);  // shaft
                    drawRect(mx,      my - 12, 8, 24, pulse, pulse, pulse); // head wing
                } else if (atRight) {
                    // Arrow pointing right
                    drawRect(mx - 20, my - 4, 20, 8, pulse, pulse, pulse);
                    drawRect(mx - 8,  my - 12, 8, 24, pulse, pulse, pulse);
                } else if (atBottom) {
                    // Arrow pointing down (off-screen below)
                    drawRect(mx - 4, my,      8, 20, pulse, pulse, pulse);
                    drawRect(mx - 12, my,     24, 8, pulse, pulse, pulse);
                } else if (atTop) {
                    // Arrow pointing up (off-screen above)
                    drawRect(mx - 4, my - 20, 8, 20, pulse, pulse, pulse);
                    drawRect(mx - 12, my - 8, 24, 8, pulse, pulse, pulse);
                } else {
                    // Fallback: just draw a big white dot at clamped position
    ...
    // ── Home platform nav marker ──────────────────────────────────────────────
    // Shown once all cells are collected but the player hasn't returned yet.
    // Projects the home platform world position using the same VP matrix as the
    // cell markers above.  Uses HOME_PLATFORM_X/WIN_Y/Z constants so moving the
    // platform only requires updating one place.
    if (collected >= total && total > 0 && !gameWon)
    {
        ...
        if (hClip.w > 0.0f)
        {
            ...
            if (fabsf(hNdc.x) <= 1.1f && fabsf(hNdc.y) <= 1.1f)
            {
                ...
                // Fast green pulse to distinguish from the blue cell markers
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // BH FALL CINEMATIC  (fadeState == 6)  —  26-second emotional descent
    //
    //  0– 5s  Tidal collapse:  ship systems die one by one over the live scene.
    //         Screen tears.  Red-shift bleeds in.  Last transmissions flash up.
    //  5– 8s  White silence:   total fade to white (event-horizon crossing).
    //         A single word appears in the white — then nothing.
    //  8–22s  The Tesseract:   infinite amber bookshelf corridor stretches away.
    //         Camera slowly drifts forward.  Ghostly versions of mission moments
    //         replay as faint amber echoes on the shelves.  Physics data scrolls.
    //         Time itself becomes visible as layered grid lines closing inward.
    // 22–26s  Singularity:     everything compresses to a single bright point.
    //         Final message pulses once, then fades.  Silence.
    // ═══════════════════════════════════════════════════════════════════════════
    if (fadeState == 6)
    {
        ...
        // ── Helper: fast deterministic pseudo-random float 0..1 ───────────────
        ...
        // ═════════════════════════════════════════════════════════════════════
        // PHASE 1  (0–5 s)  Tidal collapse — overlaid on the live 3-D scene
        // ═════════════════════════════════════════════════════════════════════
        if (T < 5.0f)
        {
            ...
            // Growing red vignette — thickens as T approaches 5
            ...
            // Screen-tear horizontal lines — random scan-line corruption
            ...
            // System failure messages — appear then cut off abruptly
            ...
            // Final line — stays on, pulsing faster and faster
            if (T > 4.0f)
            {
        ...
        // ═════════════════════════════════════════════════════════════════════
        // PHASE 2  (5–8 s)  Event-horizon crossing — white silence
        // ═════════════════════════════════════════════════════════════════════
        if (T >= 5.0f && T < 8.0f)
        {
            // The HDR fade handles the white-out; here we add one word
            float wordT = glm::clamp((T - 5.5f) / 0.5f, 0.0f, 1.0f)
                        * glm::clamp(1.0f - (T - 6.8f) / 0.8f, 0.0f, 1.0f);
            if (wordT > 0.01f)
            {
                // Single centred word — dark on white background
        ...
        // ═════════════════════════════════════════════════════════════════════
        // PHASE 3  (8–22 s)  The Tesseract — amber infinite-corridor library
        // ═════════════════════════════════════════════════════════════════════
        if (T >= 8.0f && T < 22.5f)
        {
            float tIn   = glm::clamp((T - 8.0f) / 1.5f, 0.0f, 1.0f);  // fade in
            ...
            // ── Corridor geometry ──────────────────────────────────────────
            // Perspective-projected shelves receding to a vanishing point.
            // Camera drifts "forward" at speed proportional to T so it feels
            // like falling through the structure.
            float drift     = (T - 8.0f) * 0.045f;  // slow at first
            const int ROWS  = 60;
            const int COLS  = 14;
            ...
            for (int fl = 0; fl < ROWS; fl++)
            {
                ...
                // Warm amber — brighter at near shelves, deep orange far away
            ...
            // ── Time-grid overlay ──────────────────────────────────────────
            // Thin concentric rectangles collapsing toward centre — represents
            // spacetime compressing.  Accelerates as T → 22.
            ...
            // ── Singularity glow (centre) ──────────────────────────────────
            // Grows slowly, then surges at T > 20
            ...
            // Amber-orange particle streaks rain down from the shelves.
            ...
            // ── Mission echo messages ──────────────────────────────────────
            // Appear at different depths/positions like memories on shelves.
            // Each line flickers in slowly, then burns.
            struct Echo2 {
                float showAt, hideAt;
                int   x, y, sz;
                float r, g, b;
            ...
            // ── Physics telemetry scrolling up the left side ───────────────
            if (T > 11.0f)
            {
        ...
        // ═════════════════════════════════════════════════════════════════════
        // PHASE 4  (22–26 s)  Singularity collapse — everything to a point
        // ═════════════════════════════════════════════════════════════════════
        if (T >= 22.0f && T < 26.0f)
        {
            drawRect(0, 0, width, height, 0.0f, 0.0f, 0.0f);
            ...
            // Everything collapses inward to the centre point
            ...
            // Central point of light — the last thing visible
            ...
            // Final message — fades in then burns out with the light
            if (T > 22.8f && T < 25.5f)
            {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // STATIC BH LOST SCREEN  (fadeState == 7)
    // Beautiful, sad, final.
    // ═══════════════════════════════════════════════════════════════════════════
    if (gameLost && fadeState == 7)
    {
        ...
        // Deep starfield — the universe continues without you
        for (int si = 0; si < 280; ++si) {
            ...
            sbr *= twk;
            int sw2 = (si % 11 == 0) ? 2 : 1;
            // Stars are cool white — not warm — feels lonely
            drawRect((int)sx, (int)sy, sw2, sw2, sbr * 0.88f, sbr * 0.90f, sbr);
        }

        // The black hole photon ring — a faint ghost, still pulling
        ...
        // Shadow disc — the black heart at the centre
        int shadowR = 88;
        drawRect(SCX2 - shadowR, SCY2 - shadowR / 3,
                 shadowR * 2, (shadowR * 2) / 3, 0.0f, 0.0f, 0.0f);

        // Inner glow ring fading to black
        ...
        // Epitaph text — centred, measured, final
        ...
        // Score epitaph — amber, slightly brighter
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // HYPERSPACE WIN CUTSCENE  (fadeState == 4)
    // 18 second Subnautica-style cinematic:
    //   0–2.5s  hyperspace streaks (pure black bg)
    //   2.5–5.2 fade white then back
    //   5.2–12s four planets zoom past, star field visible
    ...
    if (fadeState == 4)
    {
        ...
        // Pure black base always
        drawRect(0, 0, width, height, 0.0f, 0.0f, 0.0f);

        // ── Persistent starfield (after initial streak phase) ─────────────────
        if (T >= 2.5f)
        {
        ...
        // ── PHASE A (0–2.5s): Hyperspace streaks ─────────────────────────────
        if (T < 2.5f)
        {
        ...
        // ── Planet helpers ────────────────────────────────────────────────────
        // Planet with latitude bands + Lambertian shading + terminator
        ...
        FlybyDef flybys[4] = {
            // 1. Rocky grey-brown (moon-like) top-right → bottom-left
            { 5.2f, 1.8f,  1.20f,-0.35f,-0.30f,1.10f, 58,
              0.30f,0.24f,0.18f, 0.65f,0.52f,0.36f, 0.55f,0.68f,0.85f },
              // 2. Red volcanic planet — fast right
              { 7.2f, 1.5f,  1.15f, 0.10f,-0.20f,0.60f, 42,
                0.40f,0.08f,0.04f, 0.85f,0.28f,0.08f, 0.90f,0.48f,0.22f },
                // 3. Large icy-blue gas giant — slow, enters top
                { 8.8f, 2.2f,  0.60f,-1.20f, 0.30f,1.15f, 80,
                  0.12f,0.18f,0.40f, 0.28f,0.55f,0.92f, 0.35f,0.68f,1.00f },
                  // 4. Dark purple rocky world — bottom-right diagonal
                  {10.5f, 1.6f,  1.20f, 0.80f,-0.15f,-0.20f,36,
        ...
        if (T >= 12.0f)
        {
            ...
            // Full photorealistic Earth rendering
            drawEarthHelper(ECX, ECY, ER, etE, drawRect);
        }

        // ── PHASE D (15.5–18s): Stabilisation panel + revelation twist ──────
        if (T >= 15.5f)
        {
...
#pragma warning(push)
#pragma warning(disable: 4996)   // fopen/fscanf: MSVC deprecation, safe here
